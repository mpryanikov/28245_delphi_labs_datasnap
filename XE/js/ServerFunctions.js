...
function TServerMethods1(connectionInfo)
{
  ...
  /*
   * @param a [in] - Type on server: Double
   * @param b [in] - Type on server: Double
   * @return result - Type on server: Double
   */
  this.Add = function(a, b) {
    var returnObject = this.executor.executeMethod('Add', "GET", [a, b], arguments[2], true, arguments[3], arguments[4]);
    if (arguments[2] == null) {
      if (returnObject != null && returnObject.result != null && isArray(returnObject.result)) {
        var resultArray = returnObject.result;
        var resultObject = new Object();
  ...
  this.Add_URL = function(a, b) {
    return this.executor.getMethodURL("Add", "GET", [a, b], arguments[2])[0];
  };
}
